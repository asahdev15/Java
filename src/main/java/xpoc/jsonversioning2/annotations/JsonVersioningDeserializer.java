...
public class JsonVersioningDeserializer extends BeanDeserializer {
    ...
    public JsonVersioningDeserializer(
        ...
        modelVersion = xpoc.jsonversioning2.annotations.Version.fromString(jsonVersionedAnnotation.value());
        BeanPropertyDefinition versionProperty = null;
        for (BeanPropertyDefinition propertyDef: beanDesc.findProperties()) {
            if ((propertyDef.hasGetter() && propertyDef.getGetter().hasAnnotation(xpoc.jsonversioning2.annotations.JsonVersionProperty.class))
                    || (propertyDef.hasField() && propertyDef.getField().hasAnnotation(xpoc.jsonversioning2.annotations.JsonVersionProperty.class))) {
    ...
    public Object deserialize(JsonParser jsonParser, DeserializationContext ctx) throws IOException {
        ...
        try {
            if (jsonVersionProperty != null && jsonNode.has(jsonVersionProperty.getName())) {
                    jsonVersion = xpoc.jsonversioning2.annotations.Version.fromString(jsonNode.get(jsonVersionProperty.getName()).asText());
            } else {
                Object jsonVersionObj = ctx.getAttribute(xpoc.jsonversioning2.annotations.Version.JsonVersionConfigDeserializing);
                if (jsonVersionObj != null) {
                    if (jsonVersionObj instanceof Version)
                        jsonVersion = (Version) jsonVersionObj;
                    else
                        jsonVersion = xpoc.jsonversioning2.annotations.Version.fromString(jsonVersionObj.toString());
                }
            }
        ...
        Optional<BeanPropertyDefinition> propertyNotInVersion = FluentIterable.from(beanDesc.findProperties())
                ...
                .filter(Predicates.not(xpoc.jsonversioning2.annotations.JsonVersioningPredicate.forPropertyDefInVersion(jsonVersion)))
